...
**확인이 끝나면 그 테스트 캘린더를 삭제한다.** 남겨두면 3단계에서 EventKit이
그것까지 읽어, 방금 내보낸 일정이 `[Work] [가족] ...`처럼 접두사가 겹친 채 되돌아온다.

(도구가 내보낸 일정에는 `[calhub]` 표식이 붙고 소스 쪽에서 이를 건너뛰므로 실제
중복 증식은 일어나지 않는다. 다만 테스트 캘린더는 지우는 편이 깔끔하다.)
...
macOS 캘린더 앱은 기본으로 **비어 있는 로컬 캘린더**를 만든다. 한국어 환경에서는
보통 `집`과 `직장`이라는 이름이다. 이름만 보고 회사 캘린더로 착각하기 쉽지만
내용은 비어 있다.

```
✓ source 'outlook-mac': 0 events     <- 이름은 맞지만 빈 로컬 캘린더
```

**Mac용 Outlook 앱은 자체 저장소를 쓰므로 EventKit에 일정을 공개하지 않는다.**
회사 일정을 EventKit으로 읽으려면 계정을 macOS 자체에 추가해야 한다.

> 시스템 설정 → 인터넷 계정 → 계정 추가 → **Microsoft Exchange**

추가되면 캘린더 앱 목록에 회사 계정 아래로 새 캘린더가 나타난다. 그 이름을
`calendars:`에 적는다.

회사 조건부 액세스 정책이 승인된 앱만 허용하는 경우 이 추가가 거부될 수 있다.
그때는 2.1절 경로 A(Graph API)로 간다.
